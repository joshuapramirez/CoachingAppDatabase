...
-- Insert messages between User 6 to User 2
INSERT INTO Messages (message_id, sender_id, receiver_id, message_text, timestamp)
VALUES
    (1, 6, 2, 'Hello, how are you?', '2023-10-13 12:00:00'),
    (2, 6, 2, 'I hope youre doing well!', '2023-10-13 12:05:00'),
    (3, 2, 6, 'Hi there! Im good, thanks!', '2023-10-13 12:10:00'),
    (4, 2, 6, 'How about you?', '2023-10-13 12:15:00');



--SELECT QUERIES

-- Query for clients AND coach relationship using INDEX
SELECT client_name, client_id
FROM CoachClientAssociations
WHERE coach_id = 3;
...
-- Query for Client Submissions
SELECT *
FROM ClientSubmissions;


-- Query for user id and password_hash of specific user using INDEX
SELECT id, password_hash, password_salt
FROM User
WHERE email = 'michael.lee@example.com';


-- Query to get all workouts associated with a client (Sarah Brown)
SELECT W.*
FROM User U
JOIN Client C ON U.id = C.user_id
JOIN ClientProgram CP ON C.id = CP.client_id
JOIN Program P ON CP.program_id = P.id
JOIN Workout W ON P.id = W.program_id
WHERE U.first_name || ' ' || U.last_name = 'Emily Adams';


-- Query for the exercises in a library using coach id
SELECT exercise_name, exercise_category, exercise_video_link, exercise_description
FROM Library
WHERE coach_id = 2;


-- Query for the Workouts in a program
SELECT *
FROM ProgramWorkouts
WHERE program_id = 3;


-- Query for all the exercise info in a specific workout ordered by 'exercise_order' where workout_id = 4
SELECT
    Order_Exercise,
    Component_Name,
    Exercise_Name,
...
WHERE Workout_ID = 4
ORDER BY Order_Exercise;


-- Query for exercises where category = 'Strength' in a specific library where coach id = 1
SELECT
    exercise_name,
    exercise_category,
    exercise_video_link,
